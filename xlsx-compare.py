...
def compare_dataframes_cell_by_cell(df1, df2):
    ...
    df1 = df1.fillna("").astype(str).apply(lambda col: col.str.strip() if col.dtype == "object" else col)
    df2 = df2.fillna("").astype(str).apply(lambda col: col.str.strip() if col.dtype == "object" else col)
    # debug info
...
print(f"... workbook1 loaded, now loading workbook2") # to show processing progress
...
print(f"... Preparing Excel writer") # to show processing progress
output_writer = pd.ExcelWriter(output_path, engine="openpyxl")
print(f"... Excel writer prepared") # to show processing progress
...
for sheet in all_sheets:
    ...
    if sheet not in file1_sheets:
        ...
        if differences.isnull().all().all():
            ...
            differences.to_excel(output_writer, sheet_name=f"df-{safe_sheet_name}", index=False, header=False)
